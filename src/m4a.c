...
// TODO: Integrate C variables when syms are properly setup in dism.
...
void m4aSoundInit(void)
{
    ...
    for (i = 0; i < NUM_MUSIC_PLAYERS; i++)
    {
        ...
        struct MusicPlayerTrack *mplayTrack = gMPlayTable[i].track;
        // either a ptr load and dereference access or casting ptr dereference
        // is required to produce matching assembler. It isn't like this in other
        // m4a.c versions???
        MPlayOpen(mplayInfo, mplayTrack, *(u8 *)&gMPlayTable[i].unk_8);
        //mplayInfo->unk_B is not set in this version for some reason.
...
// m4aMPlayFadeOutTemporarily and m4aMPlayFadeIn should be right here, but they aren't
// in this version of m4a.
                ...
                if ((channels->statusFlags & SOUND_CHANNEL_SF_ENV) == SOUND_CHANNEL_SF_ENV_RELEASE)
                {
                    ...
                    envelope_pseudoecho_start:
                        ...
                        if (channels->envelopeVolume)
                        {
                            ...
                            channels->modify |= CGB_CHANNEL_MO_VOL;
                            // the if(ch != 3) is missing here too.
                            goto envelope_complete;
                        }
                        else
                        {
